...
export class AssetsDataSource {
    ...
    #prefix;
    #cache = new Map();
    constructor(assets, origin, prefix = "/v1") {
        this.#assets = assets;
    ...
    async read(path) {
        ...
        const response = await this.#assets.fetch(new Request(`${this.#origin}${this.#prefix}/${key}`));
        if (!response.ok) {
            this.#cache.set(key, null);
            return null;
        }
...
export class MemoryDataSource {
    #entries;
    constructor(entries) {
        this.#entries = new Map(Object.entries(entries).map(([key, value]) => [normalize(key), value]));
    }
    async read(path) {
